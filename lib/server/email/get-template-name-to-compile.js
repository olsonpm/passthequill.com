...
import { baseUrl, emailTemplateDir } from 'project-root/config/app'
...
function readTemplates(fileName) {
  return resolveAllProperties({
    html: readFile(path.resolve(emailTemplateDir, `${fileName}.inline.html`)),
    text: readFile(path.resolve(emailTemplateDir, `${fileName}.txt`)),
  })
}
