...
import { mailHost, smtpAuth } from 'project-root/config/app'
...
function createTemplateNameToSend() {
  ...
  function toApproveThenSend(approveThenCompileHtmlAndText, templateName) {
    ...
    return function approveThenSend(...arrayOfArguments) {
      ...
      const handleServerError = createHandleServerError(createErrorMessage, [
        type,
        emailSentHash,
        input,
      ])
...
function handleSendError(error) {
  return handleServerErrorSync('occurred while sending an email', error)
}
